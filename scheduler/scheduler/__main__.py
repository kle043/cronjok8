...
def create_job(batch_client, name, job_id):
    print(f'Start Creating job {name}')
    JOB_TEMPLATE['metadata']['name'] = name
    JOB_TEMPLATE['spec']['template']['spec']['containers'][0]['command']=['worker', str(job_id)]
...
def main(args=None):
    ...
    current_jobs = batch_client.list_namespaced_job(NAMESPACE,  label_selector='jobgroup=worker-group')

    # Clean finished jobs, so that they can be restarted
    finished_job_names = [j.metadata.name for j in current_jobs.items if j.status.active is None]
    ...
    running_job_names = [j.metadata.name for j in current_jobs.items if j.status.active is not None]
    ...
    for job_id, name in zip(new_jobs['ids'], new_jobs['names']):
        ...
        if k8_name not in running_job_names:
            create_job(batch_client, k8_name, job_id)
        else:
